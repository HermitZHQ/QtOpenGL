...
void OpenWidget::SimpleComputeShaderTest() {
	...
	if (1) {
		...
		// gl_WorkGroupID，当时工作的group ID（dispatch设置的，比如4，4，0），那么它的范围是[0, vec3(4, 4, 0))，也就是3，3，0，因为4个的话，和for一样，范围就是0-3
		// 
		...
		// 
		// gl_LocalInvocationID，本地（shader size）调用ID
		// gl_LocalInvocationIndex，This is a 1D version of gl_LocalInvocationID
